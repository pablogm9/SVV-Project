...
"q_b_i"
def q_b1(s):
    return M*(t*r**2*(-cos(s)+1)+A_st*sum(y_st[4:6]))

def q_b2(s):
    return M*(t_sp*s**2/2)

def q_b3(s): 
    return M*(t*(r*s-r/l*s**2/2)+A_st*sum(y_st[0:7]))+q_b1(np.pi/2)+q_b2(r)

def q_b4(s):
    return M*(t*(-r/l*s**2/2)+A_st*sum(y_st[0:6]+y_st[7:11]))+q_b3(l)

def q_b5(s):
    return M*(t_sp*s**2/2+A_st*sum(y_st[0:6]+y_st[7:11]))+q_b4(l)

def q_b6(s):
    return M*(t*r**2*(-cos(s)+1)+A_st*sum(y_st))+q_b4(l)-q_b5(-r)

"q_s0"
#integrated over s
q_1 = M*(t*r**2*(-sin(np.pi/2)+np.pi/2+sin(0))+A_st*sum(y_st[4:6])*np.pi/2)
q_2 = M*(t_sp*r**3/6)
q_3 = M*(t*(r*l**2/2-r/l*l**3/6)+A_st*sum(y_st[0:7])*l)+(q_b1(np.pi/2)+q_b2(r))*l
q_4 = M*(t*(r/l*l**3/6)+A_st*sum(y_st[0:6]+y_st[7:11])*l)+q_b3(l)*l
q_5 = M*(t_sp*r**3/6+A_st*sum(y_st[0:6]+y_st[7:11])*r)+q_b4(l)*r
q_6 = M*(t*r**2*(-sin(0)+sin(-np.pi/2)+np.pi/2)+A_st*sum(y_st)*(np.pi/2))+(q_b4(l)-q_b5(-r))*np.pi/2
...
qlist = []
q_1s = M*(t*r**2*(-np.pi/2*sin(np.pi/2)-cos(np.pi/2)+(np.pi/2)**2/2+cos(0))+A_st*sum(y_st[4:5])*(np.pi/2)**2/2)
qlist.append(q_1s)
q_2s = M*(t_sp*r**4/24)
qlist.append(q_2s)
q_3s = M*(t*(r*l**3/6-r/l*l**4/24)+A_st*sum(y_st[0:7])*l**2/2)+(q_b1(np.pi/2)+q_b2(r))*l**2/2
qlist.append(q_3s)
q_4s = M*(t*(r/l*l**4/24)+A_st*sum(y_st[0:6]+y_st[7:11])*l**2/2)+q_b3(l)*l**2/2
qlist.append(q_4s)
q_5s = M*(t_sp*r**4/24+A_st*sum(y_st[0:6]+y_st[7:11])*r**2/2)+q_b4(l)*r**2/2
qlist.append(q_5s)
q_6s = M*(t*r**2*(-cos(0)-np.pi/2*sin(-np.pi/2)+cos(np.pi/2)+(np.pi/2)**2/2)+A_st*sum(y_st)*(np.pi/2)**2/2)+(q_b4(l)-q_b5(-r))*(np.pi/2)**2/2
qlist.append(q_6s)
