...
def matrix_solver(c,L,x1,x2,x3,xa,theta,t_st,t_sk,t_sp,w_st,ha,Py,Pz,d1,d2,d3,G,E,Izz,Iyy,zshear, S,D, Td, DTd1,DTd2,DTd3,DTd4, FI1, FI2, FI3):
    ...
    Y=[D+Py*Macaulay(L-(x2+xa/2)),  #Mz(L)=0
       ...
       -Td-(0-zshear)*Py*Macaulay_power_0(L-(x2+xa/2))+(ha/2)*Pz*Macaulay_power_0(L-(x2+xa/2)),    #T(L)=0
       S+Py*Macaulay_power_0(L-(x2+xa/2)),      #Sy(L)=0
       -Pz*Macaulay_power_0(L-(x2+xa/2)),       #Sz(L)=0
       d1*cos(theta) +FI1/(Izz*E) - Py/(E*Izz)*1/6*Macauley(x1-(x2+xa/2))**3 +abs(zshear-0)*DTd1/(G*J) + abs(zshear-0)*(0-zshear)/(G*J)*Py/6*Macauley(x1-(x2+xa/2))**3 - abs(zshear-0)*ha/2*1/(G*J)*Pz/6*Macauley(x1-(x2+xa/2))**3,      #v(x1) - twist(x1)*abs(zshear-0) = d1*cos(theta)
       FI2/(Izz*E) - Py/(E*Izz)*1/6*Macauley(x2-(x2+xa/2))**3 +abs(zshear-0)*DTd2/(G*J) + abs(zshear-0)*(0-zshear)/(G*J)*Py/6*Macauley(x2-(x2+xa/2))**3 - abs(zshear-0)*ha/2*1/(G*J)*Pz/6*Macauley(x2-(x2+xa/2))**3,                     #v(x2) - twist(x2)*abs(zshear-0) = 0
       d3*cos(theta) +FI3/(Izz*E) - Py/(E*Izz)*1/6*Macauley(x3-(x2+xa/2))**3 +abs(zshear-0)*DTd3/(G*J) + abs(zshear-0)*(0-zshear)/(G*J)*Py/6*Macauley(x3-(x2+xa/2))**3 - abs(zshear-0)*ha/2*1/(G*J)*Pz/6*Macauley(x3-(x2+xa/2))**3,      #v(x3) - twist(x3)*abs(zshear-0) = d3*cos(theta)
...
#The equation becomes XX*RES = Y
    XX=np.matrix([[Macaulay(L-x1),Macaulay(L-x2),Macaulay(L-x3),0,0,0,-Macaulay(L-(x2-xa/2)),0,0,0,0,0,0], 
                  ...
                  [- (0-zshear)*Macaulay_power_0(L-x1), -(0-zshear)*Macaulay_power_0(L-x2), -(0-zshear)*Macaulay_power_0(L-x3),0,0,0,(0-zshear)*Macaulay_power_0(L-(x2-xa/2)), -(ha/2)*Macaulay_power_0(L-(x2-xa/2)),0,0,0,0,0],
                  ...
                  [-1/(E*Izz)*1/6*(Macauley(x1-x1)**3)+abs(zshear-0)*1/(G*J)*(0-zshear)*Macaulay(x1-x1)**1, -1/(E*Izz)*1/6*(Macauley(x1-x2)**3)+ abs(zshear-0)*1/(G*J)*(0-zshear)*Macaulay(x1-x2)**1, 0, 0,0,0, 0,0,x1,1,0,0,1],
                  [-1/(E*Izz)*1/6*(Macauley(x2-x1)**3)+abs(zshear-0)*1/(G*J)*(0-zshear)*Macaulay(x2-x1)**1, -1/(E*Izz)*1/6*(Macauley(x2-x2)**3)+ abs(zshear-0)*1/(G*J)*(0-zshear)*Macaulay(x2-x2)**1, 0, 0,0,0, 1/(E*Izz)*1/6*Macauley(x2-(x2-xa/2))**3 -abs(zshear-0)/(G*J)*(0-zshear)*Macaulay(x2-(x2-xa/2))**1,+abs(zshear-0)/(G*J)*(ha/2)*Macaulay(x2-(x2-xa/2))**1,x2,1,0,0,1],
                  [-1/(E*Izz)*1/6*(Macauley(x3-x1)**3)+abs(zshear-0)*1/(G*J)*(0-zshear)*Macaulay(x3-x1)**1, -1/(E*Izz)*1/6*(Macauley(x3-x2)**3)+ abs(zshear-0)*1/(G*J)*(0-zshear)*Macaulay(x3-x2)**1, -1/(E*Izz)*1/6*(Macauley(x3-x1)**3)+abs(zshear-0)*1/(G*J)*(0-zshear)*Macaulay(x3-x1)**1, 0,0,0, 1/(E*Izz)*1/6*Macauley(x3-(x2-xa/2))**3 -abs(zshear-0)/(G*J)*(0-zshear)*Macaulay(x3-(x2-xa/2))**1,+abs(zshear-0)/(G*J)*(ha/2)*Macaulay(x3-(x2-xa/2))**1,x3,1,0,0,1],
...
def torque_x(x,Td,zshear,R1y,R2y,R3y,Ay,Az,Py,Pz,x1,x2,x3,xa,ha):
    T = Td - (0-zshear)*R1y*Macaulay_power_0(x-x1) - (0-zshear)*R2y*Macaulay_power_0(x-x2) - (0-zshear)*R3y*Macaulay_power_0(x-x3) + (0-zshear)*Py*Macaulay_power_0(x-(x2+xa/2)) - (ha/2)*Pz*Macaulay_power_0(x-(x2+xa/2)) - (ha/2)*Az*Macaulay_power_0(x-(x2-xa/2)) + (0-zshear)*Ay*Macaulay_power_0(x-(x2-xa/2))
    return(T)
